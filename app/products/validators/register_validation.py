...
class ValidationProductRegistration:
    def __init__(self, db, code, product, cost_payment, date_limited, category, converter_format):
        self.datenow = datetime.now()
        self.db = db
        self.code = code
        self.product = product
        self.cost = cost_payment
        self.datelimited = converter_format(date_limited)
    ...
    def date_in_range(self):
        ...
        if self.datelimited < self.datenow: # validacion de fecha del producto con fecha actual > aun no
